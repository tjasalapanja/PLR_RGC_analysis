...
%sortResponses
areaAll=zeros(length(offROI),1); 
for roi=1:length(offROI)
    areaAll(roi)=trapz(abs(offROI(roi).mean_flash(flashParams.framesBeforeFlash:flashParams.framesBeforeFlash+flashParams.framesDuringFlash,3)));
end
[~, ind] = sort(areaAll,'descend');
sortedResp=zeros(length(offROI),145,3);% rois,flashes

c=1;
for a=ind'
    sortedResp(c,:,:)=offROI(a).mean_flash(10:154,[1:3]);
    c=c+1;
end
