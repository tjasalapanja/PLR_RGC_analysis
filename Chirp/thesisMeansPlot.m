function thesisMeansPlot(allROI,chirpParams,chirpClass)
    ...
    figure('Color','w','Position',[100,100,500,750])
...
%     colorDark=crameri('hawaii',6);
    ...
    for c=1:5
        ...
        nexttile(plotOrder(c),[1,1])
        plotOne(allROI(classList(listOfROI(c))),chirpParams,colorToUse,limitsExContra(c,:),[2])
        xlim([seconds(1),seconds(end)])
        box off
        xticklabels({''})
        yticks([0,limitsExContra(c,2)])
        xlabel('')
        hold on
        ax=gca;
        ax.Color='w';
        ...
        nexttile(plotOrder(c)+2,[1,1])
        plotOne(pop,chirpParams,colorToUse,limitsPopContra(c,:),2)% t=2
        xlim([seconds(1),seconds(end)])
        ...
        nexttile(plotOrder(c)+1,[1,1])
        plotOne(allROI(classList(listOfROI(c))),chirpParams,colorToUse,limitsExIpsi(c,:),[3])
        hold on
        xlim([seconds(1),seconds(end)])
        yticks([0,limitsExIpsi(c,2)])
        xticklabels({''})
        xlabel('')
        box off
        ax=gca;
        ...
        nexttile(plotOrder(c)+3,[1,1])
        plotOne(pop,chirpParams,colorToUse,limitsPopIpsi(c,:),3)% t=2
        hold on
        xlim([seconds(1),seconds(end)])
        set(gca,'FontSize',9)
