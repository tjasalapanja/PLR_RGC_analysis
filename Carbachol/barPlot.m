...
maxPeakDuringContra=zeros(length(OFFhighPost),1);
maxPeakDuringIpsi=zeros(length(OFFhighPost),1);
maxPeakBeforeContra=zeros(length(OFFhighPost),1);
maxPeakBeforeIpsi=zeros(length(OFFhighPost),1);

for roi=1:length(OFFhighPost)
    maxPeakBeforeContra(roi)=max(OFFhighPost(roi).mean_flash(18:flashParams.framesBeforeFlash,2));
    maxPeakBeforeIpsi(roi)=max(OFFhighPost(roi).mean_flash(18:flashParams.framesBeforeFlash,3));
    maxPeakDuringContra(roi)=max(OFFhighPost(roi).mean_flash(flashParams.framesBeforeFlash+1:flashParams.framesBeforeFlash+flashParams.framesDuringFlash,2));
    maxPeakDuringIpsi(roi)=max(OFFhighPost(roi).mean_flash(flashParams.framesBeforeFlash+1:flashParams.framesBeforeFlash+flashParams.framesDuringFlash,3));
end
