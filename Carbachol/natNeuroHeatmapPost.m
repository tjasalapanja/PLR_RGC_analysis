...
areaOFF=zeros(length(OFFroiHighPost),1); 
for roi=1:length(OFFroiHighPost)
    areaOFF(roi)=trapz(abs(OFFroiHighPost(roi).mean_flash(flashParams.framesBeforeFlash:flashParams.framesBeforeFlash+flashParams.framesDuringFlash,3)));
end
[~, ind] = sort(areaOFF,'descend');
sortedResp=zeros(length(OFFroiHighPost),111,3);% rois,flashes

c=1;
for a=ind'
    sortedResp(c,:,:)=OFFroiHighPost(a).mean_flash(31:141,[1:3]);
    c=c+1;
end
