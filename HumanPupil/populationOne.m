function populationOne(ROI_list,flashParams,cmap,type,limy)
    if length(ROI_list(:,1))>1
        count=1;
        data=zeros(length(ROI_list),size(ROI_list(1).meanSlow,1),size(ROI_list(1).meanSlow,2));
        for roi=1: length(ROI_list)
            if sum(ROI_list(roi).respondingROI)>0
                data(roi,:,:)=ROI_list(roi).meanSlow;
                count=count+1;
            else
                data(roi,:,:)=nan;
            end
        ...
        pop.mean_flash=zeros(size(ROI_list(1,1).meanSlow,1),size(ROI_list(1,1).meanSlow,2)); 
        pop.std_flash=zeros(size(ROI_list(1,1).meanSlow,1),size(ROI_list(1,1).meanSlow,2));
        ...
        maxlims=maxlim;
        minlims=minlim;
        %limy=[min(minlims)-0.1,max(maxlims+0.1)];
        plotOne(pop,flashParams,cmap,limy,type)
    end