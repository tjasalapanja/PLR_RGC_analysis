function plotExamplePupil(LL,flashParams1,cmap,limy,types,patchRange,lines,patchVis,example)
    ...
    hold on
    for type=types
        plot(seconds, LL.all_zscore(:,example,type), 'color',cmap(type,:),'LineWidth',lines);
%         seconds2 = [seconds, fliplr(seconds)];
%         areaStd= [LL.meanZscore(:,type)'+LL.semZscore(:,type)', fliplr(LL.meanZscore(:,type)'-LL.semZscore(:,type)')];
