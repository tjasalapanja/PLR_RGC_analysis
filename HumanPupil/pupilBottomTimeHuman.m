...
data=squeeze(pupilHuman.meanTraces(:,60:264))';
% Number of repetitions
num_repetitions = size(data, 2);

% Initialize inflection point positions
inflection_pointsHuman = zeros(1, num_repetitions);
...
for i = 1:num_repetitions
    ...
    [~, max_index] = max(d2y);
    inflection_pointsHuman(i) = x(max_index);
end

% Calculate mean and standard deviation of inflection point positions
mean_inflection_point = mean(inflection_pointsHuman);
std_inflection_point = std(inflection_pointsHuman);
