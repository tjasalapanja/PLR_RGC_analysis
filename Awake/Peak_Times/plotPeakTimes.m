...
sortedResp=zeros(length(ipsi),130,2);% rois,flashes
c=1;
for a=ind'
    sortedResp(c,:,:)=ipsi(a).mean_flash(31:160,[2,3]);
    c=c+1;
end
%Find Peak Times
...
nexttile(1)
h=heatmap(sortedResp(:,:,1),'grid','off','Colorlimits',[-0.1,1.5]);
map=contraColor;
h.Colormap = map;
h.ColorbarVisible =0;
Ax = gca;
...
ylabel('Contra')
nexttile(2)
h=heatmap(sortedResp(:,:,2),'grid','off','Colorlimits',[-0.1,1.5]);
map=ipsiColor;
h.Colormap = map;
h.ColorbarVisible =0;
Ax = gca;
