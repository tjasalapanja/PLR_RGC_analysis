...
class=find(pca.clsKmeans==2);
offROI=respondingAll(class,:);
% get colorscheme
cmap=[175,88,186;0,205,108;125,125,125]./255;
% get Max value during and before (in case I use background -during)
maxPeakDuringContra=zeros(length(offROI),1);
maxPeakDuringIpsi=zeros(length(offROI),1);
maxPeakBeforeContra=zeros(length(offROI),1);
maxPeakBeforeIpsi=zeros(length(offROI),1);

for roi=1:length(offROI)
    maxPeakBeforeContra(roi)=max(offROI(roi).mean_flash(18:flashParams.framesBeforeFlash,2));
    maxPeakBeforeIpsi(roi)=max(offROI(roi).mean_flash(18:flashParams.framesBeforeFlash,3));
    maxPeakDuringContra(roi)=max(offROI(roi).mean_flash(flashParams.framesBeforeFlash+1:flashParams.framesBeforeFlash+flashParams.framesDuringFlash,2));
    maxPeakDuringIpsi(roi)=max(offROI(roi).mean_flash(flashParams.framesBeforeFlash+1:flashParams.framesBeforeFlash+flashParams.framesDuringFlash,3));
end
