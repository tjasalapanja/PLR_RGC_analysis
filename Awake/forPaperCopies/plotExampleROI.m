...
for roi=161:170%OFF boutons 48,53  %455,423
    ...
    ax.XColor ='w';
    nexttile(2,[4,1])
    lims=[-0.7,2.5];% OFF [-0.1,4.5],ON1=[-0.8,0.9],ON2=[0,6,0.9]
    plotOneSEM(respondingROI(roi),flashParams,cmap,lims,[2],0.05,1.5) %[2:3] or [1:3], SEM or SD
    ylabel('dF/F')
    ax=gca;
    ax.Color='w';
    ax.YColor = 'w';
    ...
    nexttile(6,[4,1])
    lims=[-0.7,2.5];% OFF [-0.1,4.5],ON1=[-0.8,0.9],ON2=[0,6,0.9]
    plotOneSEM(respondingROI(roi),flashParams,cmap,lims,[3],0.05,1.5) %[2:3] or [1:3], SEM or SD
    ylabel('dF/F')
    ax=gca;
    ax.Color='w';
    ax.YColor = 'w';
